      ...
      foreach ($tables as $row){
        if (is_array($_POST['tables']) && count($_POST['tables'])>0 && in_array($row[0], $_POST['tables'])) {
          ...
          if(!file_exists(KISS_Framework_Config::getSystemPath('class')."/".KISS_Util_Util::magicName($row[0])."2.php")) {
            ...
            $fields = mysql_list_fields($db_configs[$i]['DatabaseName'], $row[0]);
            $columns = mysql_num_fields($fields);
            for ($j = 0; $j < $columns; $j++) {
              $keys[$j] = array("field"=>mysql_field_name($fields, $j),"type"=>$this->getPhpType(mysql_field_type($fields, $j)),"desc"=>mysql_field_name($fields, $j),"member"=>implode("", array_map('ucfirst', explode('_', mysql_field_name($fields, $j)))));
              for($k=0;$k<count($table_config);$k++) {
                if($table_config[$k]['name']==mysql_field_name($fields, $j))
                $keys[$j]['desc']=$table_config[$k]['comment'];
              }
            }

            $this->mSmarty->assign('keys', $keys);
            if (empty($row[1]))
            $this->generate_file(KISS_Framework_Config::getSystemPath('class')."/".KISS_Util_Util::magicName($row[0]).".php", $this->mSmarty->fetch("base/tpl.class.view.mapping.htm"));
            else
            $this->generate_file(KISS_Framework_Config::getSystemPath('class')."/".KISS_Util_Util::magicName($row[0]).".php", $this->mSmarty->fetch("base/tpl.class.mapping.htm"));
          }
  ...
  private function config() {
    ...
    if(isset($_GET['table']) && isset($_GET['config'])) {
      ...
      if(!file_exists($config_file)) {
        $SqlCommand = &KISS_KDO_SqlCommand::getInstance($_GET['config']);
