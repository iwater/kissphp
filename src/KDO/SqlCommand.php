...
abstract class KISS_KDO_SqlCommand extends KISS_Object
{
    ...
    function ExecuteCountQuery ($pSql)
    {
        ...
        return intval($this->ExecuteScalar($count_sql));
    }
    ...
    function ExecuteAssocArrayQuery ($sql, $pPageNo = 0, $pPageSize = 10)
    {
        return $this->ExecuteArrayQuery($sql, $pPageNo, $pPageSize, 'assoc');
    }
    ...
    public function ExecuteSingleArrayQuery ($sql, $pPageNo = 0, $pPageSize = 10, $pResultType = 'both')
    {
        return array_pop($this->ExecuteArrayQuery($sql, $pPageNo, $pPageSize, $pResultType));
    }
    public function ExecuteSingleAssocArrayQuery ($sql, $pPageNo = 0, $pPageSize = 10, $pResultType = 'both')
    {
        return array_pop($this->ExecuteAssocArrayQuery($sql, $pPageNo, $pPageSize, $pResultType));
    }
    ...
    function ExecuteQuery ($pQuery)
    {
        ...
        if (KISS_Framework_Config::getMode() == 'debug') {
            KISS_Util_Debug::setDebugInfo(array(
                $this->UniqueObjectID, 
                get_class($this), 
                'SQLQuery', 
                $pQuery));
